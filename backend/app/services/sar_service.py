...
def get_dataset_summary(db_id: str) -> Dict[str, Any]:
    ...
    # ✅ 新增：按你当前 schema_dict 结构统计表数/字段数
    tables_count = None
    columns_count = None
    try:
        schema_dict = load_schema_dict(db_id)  # schema.json: {tables, columns, foreign_keys}
        tables_count = len(schema_dict.get("tables", []))
        columns_count = sum(len(v) for v in (schema_dict.get("columns", {}) or {}).values())
    except FileNotFoundError:
        # 没 parse 过 schema，就保持 None，让前端显示 '-'
        pass

    # ✅ 返回给 Dashboard：它会优先读 tables_count / columns_count
    return {
        "db_id": db_id,
        "dataset": dataset,
        "index": index,
