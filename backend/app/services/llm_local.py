...
def _get_model():
    global _model
    if _model is None:
        model_path = os.getenv("LOCAL_MODEL_PATH", "/path/to/localmodel")
        _model = ModelInterface(model_name=model_path)  # 你已有这个类 :contentReference[oaicite:5]{index=5}
    return _model

def LLM_generation(instruct, prompt, n=5, **kwargs):
    mi = _get_model()
    outs = mi.LLM_generation(instruct=instruct, prompt=prompt, n=n, **kwargs)
    ...
    for s in outs:
        ...
        if not s:
            continue
        # 包成 final_generate_service.extract_sql_content 能识别的格式
        wrapped.append(f"```sql\n{s}\n```")
    return wrapped
